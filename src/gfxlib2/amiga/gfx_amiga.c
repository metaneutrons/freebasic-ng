...
static void driver_set_palette(int index, int r, int g, int b) {
    if (amiga_screen && GfxBase) {
        /* Scale 0-63 to 32-bit (SetRGB32 expects 0xFFFFFFFF range) */
        /* Scale 6-bit (0-63) to 32-bit by replicating bits */
        ULONG r32 = ((ULONG)r << 26) | ((ULONG)r << 20) | ((ULONG)r << 14) | ((ULONG)r << 8) | ((ULONG)r << 2);
        ULONG g32 = ((ULONG)g << 26) | ((ULONG)g << 20) | ((ULONG)g << 14) | ((ULONG)g << 8) | ((ULONG)g << 2);
        ULONG b32 = ((ULONG)b << 26) | ((ULONG)b << 20) | ((ULONG)b << 14) | ((ULONG)b << 8) | ((ULONG)b << 2);
...
static void driver_update(void) {
    ...
    /* Allocate/reallocate temp bitmap if needed */
    if (!tmp_bm || tmp_bm_width != w) {
        if (tmp_bm) FreeBitMap(tmp_bm);
        InitRastPort(&tmp_rp);
        tmp_bm = AllocBitMap(w, 1, amiga_screen->RastPort.BitMap->Depth, 0, NULL);
    ...
    while ((msg = (struct IntuiMessage *)GetMsg(amiga_window->UserPort))) {
        switch (msg->Class) {
            ...
            case IDCMP_MOUSEBUTTONS:
                ...
                break;
            case IDCMP_RAWKEY: {
                int scancode = msg->Code & 0x7F;
                int pressed = !(msg->Code & 0x80);
                /* Update internal key state for MultiKey */
                __fb_gfx->key[scancode] = pressed ? TRUE : FALSE;
                /* Post key event for Inkey$/GetKey (press only) */
                if (pressed) fb_hPostKey(scancode);
                break;
            }
        }
