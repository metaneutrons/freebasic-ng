...
FBSTRING *fb_ConsoleInkey(void) {
    ...
    if (dos_Read(in, &keybuf[0], 1) != 1)
        return fb_StrAllocTempDescZEx(NULL, 0);
    len = 1;
    if (keybuf[0] == 27 && dos_WaitForChar(in, 100000)) {
        /* Escape sequence - read remaining bytes */
        if (dos_Read(in, &keybuf[1], 1) == 1) {
            len = 2;
            if (keybuf[1] == '[' && dos_WaitForChar(in, 100000)) {
                if (dos_Read(in, &keybuf[2], 1) == 1) len = 3;
            }
        }
    }
    keybuf[len] = 0;
    return fb_StrAllocTempDescZEx(keybuf, len);
}
