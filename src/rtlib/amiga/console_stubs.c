...
int fb_ConsoleLocate(int row, int col, int cursor) {
    ...
    if (row > 0 && col > 0) {
        char r[12], cl[12];
        int_to_str(row, r); int_to_str(col, cl);
        buf[0] = '\033'; buf[1] = '['; buf[2] = 0;
        strncat(buf, r, sizeof(buf)-3); strncat(buf, ";", sizeof(buf)-strlen(buf)-1);
        strncat(buf, cl, sizeof(buf)-strlen(buf)-1); strncat(buf, "H", sizeof(buf)-strlen(buf)-1);
        con_write(buf);
        con_y = row; con_x = col;
    } else if (row > 0) {
        char r[12]; int_to_str(row, r);
        buf[0] = '\033'; buf[1] = '['; buf[2] = 0;
        strncat(buf, r, sizeof(buf)-3); strncat(buf, ";H", sizeof(buf)-strlen(buf)-1);
        con_write(buf);
        con_y = row;
    } else if (col > 0) {
        char cl[12]; int_to_str(col, cl);
        buf[0] = '\033'; buf[1] = '['; buf[2] = ';'; buf[3] = 0;
        strncat(buf, cl, sizeof(buf)-4); strncat(buf, "H", sizeof(buf)-strlen(buf)-1);
        con_write(buf);
        con_x = col;
    }
    return 0;
...
FBSTRING *fb_ConsoleInkey(void) {
    ...
    if (dos_Read(in, &keybuf[0], 1) == 1) {
        keybuf[1] = 0;
        return fb_StrAllocTempDescZEx(keybuf, 1);
    }
    return fb_StrAllocTempDescZEx(NULL, 0);
}
